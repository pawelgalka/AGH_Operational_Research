...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 28 18:32:20 2019
...
def arithmetic_rank(matrix,known_dict):    
    known_values = numpy.array(list(known_dict.values()))
    k = known_values.shape[0]
    n = matrix.shape[0]
    
    check_if_solution_exists(matrix,koczkodaj(matrix), matrix.shape[0], known_values.shape[0])
    if (check_if_rearrangement_needed(matrix,known_dict)):
        matrix = rearranged_matrix(matrix,known_dict)
    matrixA = matrix[0:n-k, 0:n-k]
    matrixB = matrix[0:n-k, n-k:n]
    A_dash = (-1/(n-1)) * matrixA + (1 + 1/(n-1))*numpy.identity(matrixA.shape[0])
    
    B_dash = (1/(n-1)) * numpy.dot(matrixB, known_values)
    
    W = numpy.linalg.solve(A_dash, B_dash)
    
    for k in known_dict.keys():
       W = numpy.insert(W,k-1,known_dict[k])
    print('Arithmetic rank', W)
...
print('Matrix A')
arithmetic_rank(A,known_A)
print('Matrix B')
arithmetic_rank(B,known_B)
print('Matrix C')
arithmetic_rank(C,known_C)
